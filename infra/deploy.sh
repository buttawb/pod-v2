...
# The Aurora writer endpoint, read from Terraform so there is one source of
# truth for it. The runtime URL and the migration URL are both built from it
# below and can never disagree: migrations landing in one database while reads
# come from another is drift that only shows up at 500-time.
DB_HOST="${DB_HOST:-$(terraform -chdir=terraform output -raw aurora_writer_endpoint)}"
...
# deploy should do behind your back. The install line is in the unit file.
rsync -az docker-compose.prod.yml Caddyfile systemd "ec2-user@${HOST}:${REMOTE_DIR}/"
...
# Only what the API actually reads. The two connection strings below are the
# only place the database credentials appear: pod_app for the runtime, which
# holds append-only grants, and pod for migrations, which need to own the
# schema. Keeping the raw passwords out of the environment means there is no
# second copy to leak into a log line or a crash dump.
env = {
    'NODE_ENV': 'production',
    'PORT': '3000',
    'DOMAIN': '${DOMAIN}',
...
# The round is selected by created_at >= date_trunc('day', now()), so a demo
# database that has not been rolled today serves an empty round to every driver
# login, which looks exactly like a broken deploy. The nightly timer covers the
# normal case; running it here covers a deploy that lands before it fires.
#
# Safe on every deploy: the SQL only shifts timestamps forward and deletes
# nothing, and a round already rolled today matches no rows.
echo "==> Rolling the demo round"
